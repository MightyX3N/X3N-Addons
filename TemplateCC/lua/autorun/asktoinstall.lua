...
local WORKSHOP_ID = 3397912717 -- Numeric ID (can also be a string)
...
if SERVER then
    ...
    local function CheckServerWorkshopAddon()
        ...
        if not isSubscribed then
            print("[X3N Color Corrections] WARNING: Required Workshop addon (ID: " .. WORKSHOP_ID .. ") is not installed on the server.")
            -- Optional: Disable certain features or notify admins
            -- Example: 
            -- hook.Remove("SomeFeatureHook", "FeatureName")
        else
            print("[X3N Color Corrections] Required Workshop addon is installed on the server.")
        end
    end
...
if CLIENT then
    ...
    local function CheckClientWorkshopAddon()
        -- Ensure steamworks library is available
        if not steamworks or not steamworks.IsSubscribed then
            print("[X3N Color Corrections] Error: steamworks.IsSubscribed is not available on the client.")
            return
        end
        ...
        if not isSubscribed then
            ...
            print("[X3N Color Corrections] X3N Color Corrections Base is installed on the client.")
        end
    end
